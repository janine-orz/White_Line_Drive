...
def ColRec(img, i, j):
    ...
    if b_val > 130 and g_val > 160 and r_val > 150:
        color = "White"
...
def LineForm(img, height, width, string1, string2, Wi_min, Wi_max, Gi_min, Gi_max):
    ...
    if(i_min == Gi_min): # The Greenline is shorter then the Whiteline
        for h in range (Wi_min, Gi_min):
            Line1_temp = ColDet(img, height, width, h, string1)
            L1 = Lineform(Line1_temp, h, h, string1, img, height, width)
            L2 = [0, h]
    ...
    for i in range(i_min, i_max):
        ...
        L1 = Lineform(Line1_temp, i, i, string1, img, height, width)
        L2 = Lineform(Line2_temp, i, i, string2, img, height, width)
        # print('\tL1 = ', L1, '\t\tL2 = ', L2)
        x_temp = MidCal(L1[0], L2[0])
        temp = [x_temp, i]
        L.append(temp)
    if(i_max == Gi_max): # The Greenline is shorter then the Whiteline
        for k in range (Gi_max, Wi_max):
            ...
            L1 = Lineform(Line1_temp, k, k, string1, img, height, width)
            L2 = [0, k]
            x_temp = MidCal(L1[0], L2[0])
            temp = [x_temp, k]
            L.append(temp)
...
def Lineform(Line_temp, i, j, string, img, height, width):
