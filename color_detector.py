...
def ColRec(img, i, j, BGR, HSV):
    ...
    if b_val > BGR[0] and g_val > BGR[1] and r_val > BGR[2]:
        ...
        if s_val >= HSV[0] and v_val >= HSV[1]:
            if h_val < 5:
                color = "Red"
            elif h_val < 10:
                color = "Orange"
...
def ColDet(img, height, width, i, BGR, HSV, string, Col): 
...
def LineForm(img, height, width, string1, string2, Wi_min, Wi_max, Gi_min, Gi_max):
    ...
    if(Wi_min == Wi_max):
        BGR = [170, 170, 170]
        diff = 155
        L = Follow_White(img, height, width, BGR, HSV, L, L1, WL, GL, Wi_min, Wi_max, Gi_min, Gi_max, "Green", diff)
        return L
    elif(Gi_min == Gi_max):
        BGR = [190, 190, 190]
        diff = 155
        L = Follow_White(img, height, width, BGR, HSV, L, L1, WL, GL, Wi_min, Wi_max, Gi_min, Gi_max, "White", diff)
        return L
    elif(Gi_min > Wi_max):
        diff = 145
        L = Follow_White(img, height, width, BGR, HSV, L, L1, WL, GL, Wi_min, Wi_max, Gi_min, Gi_max, "White", diff)
    ...
    for i in range(i_min, i_max):
        ...
        # print("==========white line==========")
        L1 = ColDet(img, height, width, i, BGR, HSV, string1, L1)
        ...
        # print("==========green line==========")
        hsv_img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        L2 = ColDet(img, height, width, i, BGR, HSV, string2, L2)
    ...
    L, WL, GL = UpperLine(img, height, width, BGR, HSV, L, L1, L2, WL, GL, string1, string2, i_min, Gi_min, Wi_min, diff_min)
    L, WL, GL = BottomLine(img, height, width, BGR, HSV, L, L1, L2, WL, GL, string1, string2, i_min, i_max, Gi_min, Gi_max, Wi_min, Wi_max, diff_max)
    ...
    if((Li[0] == -100)):
        ...
        if(L[0][0] == -100):
            if(string == 'White'):
                ...
                HSV = [100, 100]
                while (Li[0] == -100):
                    print("------------ While loop L[0][0] == -100 ------------")
                    L = ColDet(img, height, width, i, BGR, HSV, string, L)
                    print("L = ", L)
                    BGR[0] = BGR[0] - 10
                    BGR[1] = BGR[1] - 10
                    BGR[2] = BGR[2] - 10
...
def Follow_White(img, height, width, BGR, HSV, L, L1, WL, GL, Wi_min, Wi_max, Gi_min, Gi_max, string1, diff):
    print("===========  Follow_White  ===========")
    if(string1 == "White"):
        i_min = Wi_min
        i_max = Wi_max
    ...
    if (string1 == "White") :
        for h in range (i_min, i_max, 1):
            L1[0] = -100
            L1 = ColDet(img, height, width, h, BGR, HSV, string1, L1)
        ...
        for h in range (i_min, i_max, 1):
            L2[0] = -100
            L2 = ColDet(img, height, width, h, BGR, HSV, string1, L2)
...
def UpperLine(img, height, width, BGR, HSV, L, L1, L2, WL, GL, string1, string2, i_min, Gi_min, Wi_min, diff):
    print("===========  UpperLine  ===========")
    if(i_min == Gi_min) and (Gi_min != 0): 
        print("(i_min == Gi_min) and (Gi_min != 0)")
        # The lower bound of Whiteline is smaller then the Greenline
        for h in range (Gi_min, Wi_min-1, -1):
            print("h = ", h)
            L1[0] = -100
            L1 = ColDet(img, height, width, h, BGR, HSV, string1, L1)
            print("L1 = ", L1)
        ...
        for h in range (Wi_min, Gi_min-1, -1):
            L2[0] = -100
            L2 = ColDet(img, height, width, h, BGR, HSV, string2, L2)
...
def BottomLine(img, height, width, BGR, HSV, L, L1, L2, WL, GL, string1, string2, i_min, i_max, Gi_min, Gi_max, Wi_min, Wi_max, diff):
    print("===========  BottomLine  ===========")
    if(Wi_max < Gi_min):
        print("i_max = ", i_max, "\ni_min = ", i_min, "\nWi_min = ", Wi_min, "\nWi_max = ", Wi_max, "\nGi_min = ", Gi_min, "\nGi_max = ", Gi_max)
        Wi_max = Gi_max
...
def LineorCurve(Line, width, img):
    slope = 0.0
    LoC = 0
    ...
    slope1 = CalSlope(Line, width, idx0, idx1)
    ...
    slope2 = CalSlope(Line, width, idx3, idx2)
...
def LineSlope(Line, width, LoC):
    ...
    print("===========  LineSLope  ===========")
    slope = CalSlope(Line, width, l, idx2)
    return slope


def CalSlope(Line, width, idx1, idx2):
